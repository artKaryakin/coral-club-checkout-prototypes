...
- **Ядро стенда** (`src/stand/*`) — что показывается: оси переключения, конфиг полей по странам, локализация, состояния кейсов, подсказки адресов, данные карты.
...
2. **Не создавать и не удалять поля формы.** Поля приходят из конфига и рендерятся через `StandField`. Если макет требует нового поля — не добавлять его в шаблон, а сообщить дизайнеру: какое поле, тип, обязательность, для каких стран, после какого поля.
...
6. **Не хардкодить подписи полей** — они приходят из локализации уже переведёнными.
