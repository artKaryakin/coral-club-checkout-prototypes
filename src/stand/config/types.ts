...
export interface FieldConfig {
  ...
  required?: boolean
  /**
   * Заполняется из подсказки или по индексу — под полем об этом сказано.
   * Поле при этом остаётся редактируемым: подсказка ошибается, и тупик
   * из заблокированного поля стоит дороже опечатки.
   */
  autofilled?: boolean
  /**
...
/**
 * Профиль пользователя — то, что в проде приходит из личного кабинета.
 *
 * На стенде личного кабинета нет, поэтому профиль заполняется на входе:
 * респондент указывает своё имя, почту и телефон, и дальше эти данные
 * подставляются получателем во все формы доставки. Без этого шага он
 * вводит чужие данные и не замечает ни ошибок автоподстановки, ни того,
 * что что-то подставилось неправильно.
 *
 * Имя и фамилия хранятся отдельно, а не одной строкой: на части рынков
 * это два разных поля формы, и склеенную строку пришлось бы разбирать
 * обратно, угадывая, где заканчивается имя.
 */
...
export interface CountryConfig {
  ...
  locale: LocaleCode
  /**
   * Название страны на её собственном языке — «Қазақстан», «Česko».
   * На первом экране стенда страна не переводится: респондент ищет свою
   * страну взглядом, и написанная на его языке она находится быстрее,
   * чем переведённая на язык интерфейса, который он ещё не выбрал.
   */
  nativeName: string
